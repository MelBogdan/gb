...
func init() {
	...
	flag.IntVar(&timeoutSec, "timeout", 3, "max timeout for parse")
	flag.Parse()
...
func main() {
	started := time.Now()
	pid := os.Getpid()
	log.Println("Own process identifier: ", strconv.Itoa(pid))
	ctx, cancel := context.WithCancel(context.Background())
	go watchSignals(cancel)
	defer cancel()

	crawler := newCrawler(depthLimit, timeoutSec)
	...
	restart:
		...
		signal.Notify(UserChan, syscall.SIGUSR1)
		<-UserChan
		log.Println("//////////////////////////////////////////////////////////////////////////////")
		crawler := newCrawler(depthLimit, timeoutSec)
