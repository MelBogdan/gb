...
func newCrawler(maxDepth, timeoutSec int) *crawler {
	return &crawler{
...
func (c *crawler) run(ctx context.Context, url string, results chan<- crawlResult, depth int) {
	...
	ctxTimeout, _ := context.WithTimeout(ctx, time.Duration(timeoutSec)*time.Second)
	...
	default:
		...
		for link := range links {
			...
			go c.run(ctxTimeout, link, results, depth+1)
		}
	}
}
